...
import com.bsoft.xds.support.instance.summary.TraumatismService;
...
@Controller
@LogonValidation
public class TraumatismHistoryListController {

	private static final Logger logger = LoggerFactory
			.getLogger(DiseaseHistoryListController.class);

	@Autowired
	private TraumatismService traumatismService;
	 ...
	 * @return
	 */
	@RequestMapping(value = "/summary/getTraumatismHistoryList", method = RequestMethod.GET)
	@ResponseBody
	public ControllerResponse getDiseaseHistoryList(VisitKey visitKey,
		...
		try {
			List<Map<String, Object>> list = traumatismService.getTraumatismService(
					visitKey.getMpiId(), start, limit);
			res.setBody(list);
		} catch (ServiceException e) {
			logger.error("Cannot get sick history record.", e);
			res.setStatus(ServiceCode.CODE_DATABASE_ERROR, "获取外伤史记录失败。");
		}
		return res;
	}
}

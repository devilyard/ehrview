...
var pregnantObj;
var cnd = {};
var BabyVisit = function(id) {
	$("#" + id).empty();
	var url = "postnatal/getBabyVisit";
...
var setUlNav = function(body, id, pId) {
	...
	for (var i = 0; i < body.length; i++) {
		...
		$("#dataNavBar-" + id + "-" + pId).append(html)
		var contentStr = "<div id='" + time + "' class='tab-pane'></div>";
		$("#content-" + id).append(contentStr)
	}

	$('#dataNavBar-' + id + '-' + pId + ' a:first').tab('show')
	var contentId = $('#dataNavBar-' + id + '-' + pId + ' a:first').attr('href');
	contentId = contentId.replace(/#/g, "");

	var url = "../" + docUrl;
	setHtmltoPage(url, $('#dataNavBar-' + id + '-' + pId + ' a:first').attr('id'), contentId)

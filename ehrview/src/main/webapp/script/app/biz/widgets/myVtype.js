...
Ext.apply(Ext.form.VTypes, {
			...
			"idCardMask" : /[0-9xX]/i,
			"contactNo" : function(v, field){
				var v = v.trim();
				var pattern=/^(\d{0,15}|\d{0,15}-|\d{0,15}-\d{0,15}-)\d{0,20}$/;
				if(!pattern.test(v)){
					this.contactNoText = "号码输入错误";
					return false;
				}else if(v.length > 15){
					this.contactNoText= "号码不能超过15位";
					return false;
				}
				return true;
			},
			"contactNoText" : this.contactNoText,
//			"contactNoMask" : /[0-9]|-/i
			
			"cardNo" : function(v, field){
				var v = v.trim();
				var pattern=/^[A-Za-z0-9]+$/;
				if(!pattern.test(v)){
					this.cardNoText = "存在无效字符"	;
					return false;
				}
				return true;
			},
			"cardNoText" : this.cardNoText
